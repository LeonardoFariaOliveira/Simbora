...
export const getStaticProps: GetStaticProps = async(context) => {
  ...
  const data = await fetch(`https://uenp.fun/turismo/api/hoteis`)

  const jsonData = await data.json()
  const places = jsonData.hoteis
...
const Hotels: NextPage = ({city, latitude, longitude, places}:any) => {
  ...
  return (
    <Layout>
        <Head>
          <title>Simbora!</title>
          <meta name="description" content="Hoteis em bandeirantes" />
          <meta name="viewport" content="initial-scale=1.0, width=device-width" />
          <meta property='og:title' content='Simbora' />
          <meta
            property='og:description'
            content='Hoteis em bandeirantes'
          />
        ...
        <div className='w-full relative green-gradient animate-bg min-h-screen'>
          ...
          <div className={`hidden xs:flex xs:flex-col absolute ${isModalHidden ? "z-[999]" : "z-[99999]"}   px-4 w-full top-[36.8rem] xs:top-[1rem] ${isPlaceeSelected ? "md:top-[34.9rem]" : "md:top-[36rem]"} ${isPlaceeSelected ? "sm:top-[32rem]" : "sm:top-[29rem]"} items-center justify-center`}>
